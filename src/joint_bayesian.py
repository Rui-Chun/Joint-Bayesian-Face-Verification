...
class MyPCA():
    ...
    def fit(self, data):
        ...
        self.G = u[:, 0:self.n_components]
        self.G = self.G.T

    def transform(self, data):
        return (self.G @ data.T).T
...
def PCA_Train(data, result_fold, n_components=2000):
    ...
    pickle.dump(pca, result_fold+"pca_model.pkl")
...
def data_pre(data):
    data = np.sqrt(data)
    temp_sum = np.repeat(np.sum(data, 1), data.shape[1]).reshape(data.shape[0], data.shape[1])
    data = np.divide(data, temp_sum)
...
def get_pair_ratios(A, G, pair_data):
    distance = []
    for idx in range(0, len(pair_data), 2):
        print('get ratio on {}/{}'.format(idx//2+1, len(pair_data)//2))
        ratio = Verify(A, G, pair_data[idx], pair_data[idx+1])
        distance.append(ratio)
